...
- [ ] **5. Make research runs durable with Cloud Tasks, Cloud Run, and realtime receipts**
...
- [ ] **6. Deploy the minimum live ADK and Parallel research slice**
...
- [ ] **7. Add evidence editing, three pathways, atomic publication, and partial recovery**
...
- [ ] **8. Build the bold Scout Card and embedded Industry Lens**
...
- [ ] **11. Deploy, harden, polish, and rehearse the complete judge journey**
  ...
  Acceptance: The public build completes the critical fan → agents → card → Industry Lens → social → claim story; a real deployed Parallel call is proven; seed/demo activity is labeled; failures are honest; automated tests pass; no secrets, broken citations, dead controls, placeholder copy, or unrelated files remain.
  Verify: Run the full lint/type/unit/contract/emulator/Python/Playwright suite, dependency/security checks, production smoke test, accessibility/performance review, and the timed three-minute demo twice—once live and once with an intentionally triggered fallback. **Visual pause 3: Tarik approves the deployed judge journey and submission screenshots.**
