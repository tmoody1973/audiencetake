...
export function createSocialStore(db: Firestore, options: { demoOnly?: boolean } = {}) {
  ...
  return {
    ...
    commitment(projectId: string, uid: string, type: CommitmentType, active: boolean, city?: string) { return db.runTransaction(async tx => { const { projectRef, project } = await projectAndPathway(tx, db, projectId); if (type === "bring_to_city" && active && !city?.trim()) throw new SocialError("city_required", "A city is required for this commitment."); const ref = db.collection("commitments").doc(`${projectId}_${uid}_${type}`); const old = (await tx.get(ref)).data() as Data | undefined; const was = old?.active === true; if (was !== active) { const counts = { ...(project[demoFields.commitments] ?? {}) }; counts[type] = inc(counts[type], active ? 1 : -1); tx.set(projectRef, { [demoFields.commitments]: counts, updatedAt: FieldValue.serverTimestamp() }, { merge: true }); } tx.set(ref, { projectId, uid, type, active, ...activityLabel, ...(city?.trim() ? { city: city.trim() } : {}), updatedAt: FieldValue.serverTimestamp(), ...(old?.createdAt ? {} : { createdAt: FieldValue.serverTimestamp() }) }, { merge: true }); return { active, type, ...(city?.trim() ? { city: city.trim() } : {}) }; }); },
