...
type Take = { id: string; uid?: string; whyItShouldGrow?: string; preferredPathwayId?: string; audienceNote?: string; active?: boolean; replyCount?: number; displayName?: string };
type Reply = { id: string; uid?: string; body?: string; active?: boolean; displayName?: string };
type Counts = { followerCount?: number; takeCount?: number; replyCount?: number; commitmentCounts?: Record<string, number>; pathwayVoteCounts?: Record<string, number>; demoLabel?: string };
...
export function ScoutSocialPanel({ card }: Props) {
  ...
  return <section className="scout-social-panel" id="audience-pulse" aria-labelledby="audience-pulse-title">
    ...
    <div className="social-primary"><button className="button-primary" type="button" aria-pressed={followed} aria-busy={busy === "follow"} disabled={busy !== null} onClick={onFollow}>{followed ? "Following" : "Follow this project"} <span>{counts.followerCount ?? 0}</span></button><small>Follow is a lightweight signal that you want updates.</small></div>
    <div className="social-grid">
      <fieldset className="commitment-list"><legend>Commitments</legend>{commitments.map(([type, label, definition]) => <div className="social-control" key={type}><button type="button" aria-pressed={Boolean(commitmentState[type])} aria-busy={busy === type} disabled={busy !== null} onClick={() => onCommitment(type)}>{commitmentState[type] ? "✓ " : ""}{label}<span>{counts.commitmentCounts?.[type] ?? 0}</span></button><small>{definition}</small>{type === "bring_to_city" && commitmentState[type] !== true ? <label>City<input value={city} onChange={(event) => setCity(event.target.value)} placeholder="e.g. Chicago" /></label> : null}</div>)}</fieldset>
      <fieldset className="pathway-votes"><legend>Which pathway should grow?</legend>{card.pathways.map((pathway) => <label key={pathway.id}><input type="radio" name={`pathway-${card.projectId}`} checked={vote === pathway.id} disabled={busy !== null} onChange={() => onVote(pathway.id)} /> <span>{pathway.label}</span><small>{counts.pathwayVoteCounts?.[pathway.id] ?? 0} votes</small></label>)}<button className="text-link" type="button" disabled={!vote || busy !== null} onClick={() => { if (vote) onVote(vote); }}>Clear my vote</button></fieldset>
    </div>
    ...
    {error ? <p className="field-error" role="alert">{error}</p> : null}<div className="social-live" aria-live="polite">{takes.length ? `${takes.length} published Take${takes.length === 1 ? "" : "s"}` : "No published Takes yet."}</div>
    <div className="takes-list" aria-label="Published Takes">{takes.map((take) => <article key={take.id}><h4>{take.displayName ?? "Audience member"}</h4><p>{take.whyItShouldGrow}</p><small>{card.pathways.find((pathway) => pathway.id === take.preferredPathwayId)?.label ?? "Pathway"} · {replies[take.id]?.length ?? take.replyCount ?? 0} replies</small><div className="replies-list">{(replies[take.id] ?? []).map((reply) => <div key={reply.id}><strong>{reply.displayName ?? "Audience member"}</strong><p>{reply.body}</p>{reply.uid === uid ? <><button type="button" onClick={() => setReplyDraft((old) => ({ ...old, [take.id]: reply.body ?? "" }))} disabled={busy !== null}>Edit reply</button> <button type="button" onClick={() => withdrawReply(take.id)} disabled={busy !== null}>Withdraw reply</button></> : null}</div>)}</div>{signedIn ? <form onSubmit={(event) => { event.preventDefault(); void saveReply(take.id); }}><label>Reply<textarea value={replyDraft[take.id] ?? ""} maxLength={600} onChange={(event) => setReplyDraft((old) => ({ ...old, [take.id]: event.target.value }))} /></label><button type="submit" disabled={busy !== null}>Reply</button></form> : null}</article>)}</div>
    <div className="social-demo-label"><strong>Audience Pulse is native-only.</strong> No external-web attention is included.{counts.demoLabel ? ` ${counts.demoLabel}` : ""}</div>
  </section>;
}
